...
def crcombine(
    ...
    copy_header: bool, optional
        whether to copy the header of the first provided file to the output, by default True.
    Returns
    -------
    np.ndarray
    ...
    if isinstance(imagelist[0], str):
        ...
        for i in imagelist:
            images.append(i)
        if copy_header:
            raise AssertionError("Can only copy headers if fits paths are provided.")
    ...
    if output_filename is not None:
        ...
        if header:
            hdu = fits.PrimaryHDU(combined, header=header)
        else:
...
def cli():
    ...
    parser.add_argument(
        "-d",
        "--detector_gain",
        help="detector gain (default 1.0)",
        type=float,
        default=1,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-g",
        "--grow",
        help="grow mask (default 1) before infilling",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-o",
        "--operator",
        help="how to combine final image (mean or median)",
        type=str,
        default="mean",
    )
    parser.add_argument(
        "-w",
        "--writesteps",
        help="write intermediate steps to fits files.",
        type=bool,
        default=False,
    )
    parser.add_argument(
        "-c",
        "--copyheader",
        help="copy header from first provided file to output.",
        type=bool,
        default=True,
    )
